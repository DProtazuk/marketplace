...
function Purchase() {
    ...
    if ($.isEmptyObject(quantity)) {
        ...
        $.ajax({
            url: "/backend/script/client/order/add-order.php",
            method: 'POST',
            dataType: 'json',
            data: {
                quantity: quantity,
            ...
            success: function (data) {
                if(data == "balance") {
                    ...
                    $("#a_href_order").attr('href', '/backend/script/client/order/downOrders.php?id='+data['id']);
    ...
    $('#downImg').on('change', function (event) {
        ...
        if (!$.isArray(myArray) || myArray.length > 9) {
            ...
            if ($.inArray(fileExtension, allowedExtensions) === -1) {
                ...
                $.ajax({
                    url: '/backend/script/client/reviews/down_img.php',
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
...
$('body').on('click', '.close-button', function (event) {
    ...
    $.ajax({
        url: '/backend/script/client/reviews/delete_img.php',
        method: 'POST',
        dataType: 'html',
        data: {
            img: img,
        ...
        success: function (data) {
            if (data === "Изображение успешно удалено.") {
                ...
                var myArray = JSON.parse($('#arrayImg').val() || []);

                var index = myArray.indexOf(img);
                if (index !== -1) {
...
function SendReviews() {
    ...
    if (rating) {
        ...
        $.ajax({
            url: '/backend/script/client/reviews/create_reviews.php',
            method: 'POST',
            dataType: 'json',
            data: {
                rating: rating,
