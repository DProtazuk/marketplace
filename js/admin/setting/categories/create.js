            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data === "false") {
                        ...
                        $(".div_svg").removeClass("d-none");
                        $(".div_check_img").addClass("d-none");
                    } else {
                        imgStatus = true;
                        $(".div_svg").addClass("d-none");

                        const file = img.prop('files')[0];
                        const fileUrl = URL.createObjectURL(file);

                        // Отображение временной картинки на странице
                        $(".div_check_img").attr('src', fileUrl).removeClass("d-none");
                        $(".errorImg").text("");
                    }
                }
            });
...
//Работа с Подкатегориями
{
    ...
    function addCategory() {
        let category = '<div class="col-12 d-flex mb-3">\n' +
            ...
            '\n' +
            '                                        <input require_onced name="email" value=""\n' +
        ...
        $(".div_add_category").append(category);
    }
}
...
//Работа с Параметрами
{
    ...
    function addParameter() {
        ...
        $(".div_add_parameter").append(parameter);
    }
...
function save() {
    ...
    if ($('input').hasClass('input_name_subcategories')) {
        $('.input_name_subcategories').each(function() {
            ...
            if(input_name_subcategories.trim() === '') {
                ...
                subcategories.push(input_name_subcategories);
            }
        });
    }
        ...
        $('.input_name_parameter').each(function() {
            ...
            if(input_name_parameter.trim() === '') {
                ...
                if (idValue === "none"){
                    ...
                    status = true;
                    let newParameter = { name: input_name_parameter, key: idValue };
                    parameter.push(newParameter);
                }
            }
        });
    }
    ...
    if(status) {
        if(imgStatus) {
            ...
            formData.append('global_categories', input_name_gl_category);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(data === "global_categories"){
                        $(".text-error").text("Категория уже существует");
                        $(".input_name_gl_category").parent().parent().addClass("border-danger border-2");
                        $(".input_name_gl_category").focus();
                    }
                    if(data === "save"){
                        alert("Успешно создано!");
                        location.href = "/page/admin/setting/categories/search";
                    }
                }
            });
        }
        else $(".errorImg").html("Не выбрана!");
    }
}
